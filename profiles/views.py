...
LinkFormSet = inlineformset_factory(
    ...
    can_delete=True,
    can_order=True,  # enable ordering in the formset
)
...
class ProfileLoginView(LoginView):
    ...
    Success redirect is controlled by Django (next=..., or
    settings.LOGIN_REDIRECT_URL), which in your project points to
    'post-login-redirect'.
    """
...
class ProfileLogoutView(View):
    """
    Explicitly log the user out, add a message, then redirect to index.

    Doing it ourselves avoids any ordering issues with session/message
    storage.
    """
...
class OwnerRequiredMixin(UserPassesTestMixin):
    """
    Mixin to ensure the current user owns the related profile for this object.
    """
...
def _is_real_formset_submission(post):
    """
    True if POST contains any inline formset field beyond the management
    keys, regardless of the formset prefix
    (e.g., 'link_set-0-url', 'form-1-DELETE').
    """
    mgmt_suffixes = {
        "TOTAL_FORMS",
        "INITIAL_FORMS",
...
class ProfileLinksEditorView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        # Bind the formset ONLY if there are real link fields present
        # (not just the management form)
        ...
        with transaction.atomic():
            ...
            if _is_real_formset_submission(request.POST):
                ...
                # 2) Bump positions to avoid unique collisions
                Link.objects.filter(profile=profile).update(
                    position=F("position") + 1000
                )

                # 3) Save forms we are keeping, in requested order
                cleaned_forms = [
                    f
                ...
                ordered_forms = sorted(
                    cleaned_forms,
                    key=lambda f: f.cleaned_data.get("ORDER") or 0,
                )

                for idx, form in enumerate(ordered_forms, start=1):
...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Welcome to OneLink! Your account has been created.",
            )
...
def post_login_redirect(request):
    ...
    user = request.user
    handle = (
        getattr(getattr(user, "profile", None), "handle", None)
        or user.username
    )
