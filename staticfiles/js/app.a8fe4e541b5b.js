  ...
  return {
    ...
    init: function(){
      ...
      if (form) {
        ...
        // Allow pressing Enter to trigger the same autosave as blur
        form.addEventListener('keydown', function(e){
            var t = e.target;
            if (!(t && t.tagName === 'INPUT')) return;

            // trigger only for title/url fields
            var name = t.getAttribute('name') || '';
            if (!/-(title|url)$/.test(name)) return;

            if (e.key === 'Enter') {
                e.preventDefault();   // Prevent accidental full form submission
                this.onLinkFieldBlur({ target: t });
            }
        }.bind(this), true);
